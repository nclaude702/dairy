...
$qnt=$_POST['quant'];
   ...
   if($quant<=49)
      {
      echo "<script>window.alert('pease online booking it not able if less than 50 litre ');window.location='../buying_yoghourt.php'</script>'";
       }

   elseif($quant>=49)
      {
        
        $sql="SELECT capacity FROM output_youghurt WHERE id='$pid'";
    ...
    if ($r['capacity']>=$quant=$_POST['quant'] AND $quant=$_POST['quant']>0 ) {
      $send="INSERT into online_buyed_product set p_id='$pid',names='$name',phone='$phone',mail='$mail', address='$address',type='$type',quantity='$qnt', price='$price',total_payment=quantity*price,date='$d'";
             $a=$connection->exec($send);
                if($a==true)
                   {
                     $insert="UPDATE output_youghurt set capacity=capacity-'$qnt' WHERE capacity>0 and id='$pid'";
                     $b=$connection->exec($insert);
             
                           if($b==true)
                              {
                                 //echo "<script>window.alert('buying successfull thank you');window.location='../buying_yoghourt.php'</script>'";
...
<TR></TR>
<TR></TR><tr><td></td><td><a href="../buying_yoghourt.php">Back</A>&nbsp&nbsp&nbsp<a href="#" onClick="window.print()">Print</a></td></tr>
</font></table>
<b> N.B: PAY BEFORE SEND YOUR PRODUCT</B></BR>
  <b> BANK ACCOUNT: 00091-660301-02</B></BR>
                               ...
                               }else{echo "<script>window.alert('your buying request does not reach to mukamira dairy try again');window.location='../buying_yoghourt.php'</script>'";}
                    }else{echo "<script>window.alert('your buying request does not reach to mukamira dairy try again');window.location='../buying_yoghourt.php'</script>'";}

              
      }
    else{
      echo "<script>window.alert('please buy available same type then buy again another type');window.location='../buying_yoghourt.php'</script>'";
        }
          }
