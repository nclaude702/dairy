  ...
  try
    {
     include("../../code/connection.php");
      $a=$con->prepare("SELECT * FROM `output_milk` WHERE `input_date` ='$entry' and milk_type='$ikivug'");
        if($a==true)
         {
           $update="UPDATE output_milk set capacity=capacity+'$litre',package=package+'$pack' WHERE input_date = '$d' and pack='$package'";
            $a=$con->exec($update);
            if($a==true)
                   {
              ...
              $insert="INSERT INTO output_milk VALUES ('$userid','','$p_id','$ikivug','$litre','$price','$d','$expired','$pack','$package','not exp') ";
                   $b=$con->exec($insert);
                       if($b==true)
                          {
